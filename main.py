...
class MNISTNet(nn.Module):
    def __init__(self,
        ...
        self.layer5 = nn.Linear(200, out_features)
        self.dropout_p = dropout_p
        self.dropout_at_eval = dropout_at_eval
        print("Drop: ", dropout_p)
...
class DropoutTrainer(Framework):
    def __init__(self, dataset_train,
        ...
        print("BS: ", self.batch_size, "LR: ", self.learning_rate, "EP: ", self.num_epochs)
        
        # You can check the Dummy Trainer above for intuition about what to do
        self.network = MNISTNet(in_features=28*28,out_features=10)
        
        #evens = list(range(0, len(dataset_train), 5))
        #dataset_train = data_utils.Subset(dataset_train, evens)
        #print("Hello World")
    ...
    def predict_probabilities(self, x: torch.Tensor, num_sample=100) -> torch.Tensor:
        ...
        #F.softmax(self.network(x)[0], dim=1)
        estimated_probability = [F.softmax(self.network(x)).detach().numpy() for _ in range(num_sample)]
        estimated_probability = np.stack(estimated_probability).mean(axis=0)
        estimated_probability = torch.from_numpy(estimated_probability)
        
        #print(x.shape[0])
        #print(estimated_probability.__class__)
        #print(len(estimated_probability))
        #print(len(estimated_probability[0]))
        #print(len(estimated_probability[0][0]))
